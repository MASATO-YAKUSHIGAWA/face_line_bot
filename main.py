...
@handler.add(MessageEvent, message=TextMessage) #テキストの場合
# テキストの場合はオウム返し
def handle_message(event):
   line_bot_api.reply_message(event.reply_token, TextSendMessage(text=event.message.text))

@handler.add(MessageEvent, message=ImageMessage) #画像の場合
def handle_image_message(event):
   ...
   push_img = b"" #b""にしているのは、画像代入のため（バイト型データを扱えるように）
   for chunk in message_content.iter_content(): 
       push_img += chunk #画像をiter_content（画像を分割）でpush_imgに順次代入
   push_img = base64.b64encode(push_img) # APIに通すためbase64エンコード
   msg = f.face_detect(push_img) #face_detect呼び出し
